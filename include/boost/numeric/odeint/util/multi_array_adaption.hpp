                    ...
                    >::type >
{
    static void resize( T1 &x1 , const T2 &x2 )
    {
        ...
        for( size_t i=0 ; i<T1::dimensionality ; ++i ) extents[i] = static_cast<int>(x2.shape()[i]);
        x1.resize( extents );
        boost::array< int , T1::dimensionality > origins;
        for( size_t i=0 ; i<T1::dimensionality ; ++i ) origins[i] = static_cast<int>(x2.index_bases()[i]);
        x1.reindex( origins );
    }
};
