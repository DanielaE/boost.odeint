...
struct dummy_stepper
{
    ...
    void do_step( System , state_type &x , time_type , time_type ) const
    {
        x[0] += 0.25;
    }
};
...
struct dummy_dense_output_stepper
{
    ...
    template< class System >
    std::pair< time_type , time_type > do_step( System )
    {
        m_x[0] += 0.25;
        m_t += m_dt;
...
struct dummy_controlled_stepper
{
    ...
    template< class Sys >
    controlled_step_result try_step( Sys , state_type &x , time_type &t , time_type &dt ) const
    {
        x[0] += 0.25;
        t += dt;
        return success;
