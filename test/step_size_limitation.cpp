...
void damped_osc( const state_type &x , state_type &dxdt , const value_type )
{
...
struct push_back_time
{
    ...
    void operator()( const State & , double t )
    {
        m_times.push_back( t );
    }
};
...
struct osc_rhs
{
    void operator()( const vector_type &x , vector_type &dxdt , const value_type & ) const
    {
        dxdt( 0 ) = x( 1 );
        dxdt( 1 ) = -x( 0 );
    }
...
struct osc_jacobi
{
    void operator()( const vector_type & , matrix_type &jacobi , const value_type & , vector_type &dfdt ) const
    {
        jacobi( 0 , 0 ) = 0;
        jacobi( 0 , 1 ) = 1;
        jacobi( 1 , 0 ) = -1;
